...
// bs004: monitor pieces 51-80 only
fn is_monitored(piece: usize) -> bool {
    (51..=80).contains(&piece)
}

// bs004: per-piece atomic counters for pieces 51-80
// Index 0 = piece 51, index 29 = piece 80
const MONITOR_BASE: usize = 51;
const MONITOR_COUNT: usize = 30;

struct MonitorCounters {
    requests: [AtomicUsize; MONITOR_COUNT],
    blocks: [AtomicUsize; MONITOR_COUNT],
    bytes: [AtomicU64; MONITOR_COUNT],
}

impl MonitorCounters {
    const fn new() -> Self {
        Self {
            requests: [const { AtomicUsize::new(0) }; MONITOR_COUNT],
            blocks: [const { AtomicUsize::new(0) }; MONITOR_COUNT],
            bytes: [const { AtomicU64::new(0) }; MONITOR_COUNT],
        }
    }

    fn idx(piece: usize) -> Option<usize> {
        if is_monitored(piece) {
            Some(piece - MONITOR_BASE)
        } else {
            None
        }
    }

    fn on_request(&self, piece: usize) {
        if let Some(i) = Self::idx(piece) {
            self.requests[i].fetch_add(1, Ordering::Relaxed);
        }
    }

    fn on_block(&self, piece: usize, bytes: u64) {
        if let Some(i) = Self::idx(piece) {
            self.blocks[i].fetch_add(1, Ordering::Relaxed);
            self.bytes[i].fetch_add(bytes, Ordering::Relaxed);
        }
    }

    fn take(&self, piece: usize) -> Option<(usize, usize, u64)> {
        Self::idx(piece).map(|i| {
            (
                self.requests[i].swap(0, Ordering::Relaxed),
                self.blocks[i].swap(0, Ordering::Relaxed),
                self.bytes[i].swap(0, Ordering::Relaxed),
            )
        })
    }
}

static MONITORS: MonitorCounters = MonitorCounters::new();
...
impl PeerHandler {
        ...
        self.state
            ...
            .with_live_mut(self.addr, "reserve_next_needed_piece", |live| {
                ...
                let n = {
                    ...
                    let streaming_pieces: Vec<_> = self
                        .state
                        .streams
                        ...
                        .collect();
                    if !streaming_pieces.is_empty() {
                        for n in streaming_pieces {
                        ...
                        for n in chunk_tracker.iter_queued_pieces(
                            &g.file_priorities,
    ...
    fn on_bitfield(&self, bitfield: ByteBufOwned) -> anyhow::Result<()> {
        ...
        // bs004: log PEER_HAS for each monitored piece in bitfield
        for p in 51..=80usize {
    ...
    async fn task_peer_chunk_requester(&self) -> anyhow::Result<()> {
        ...
        loop {
            ...
            for chunk in self.state.lengths.iter_chunk_infos(next) {
                ...
                // bs004: count requests for pieces 51-80
                MONITORS.on_request(request.index as usize);
                if self
                    .tx
    ...
    async fn on_received_piece(&self, piece: Piece<ByteBuf<'_>>) -> anyhow::Result<()> {
        ...
        // bs004: count blocks for pieces 51-80
        MONITORS.on_block(piece.index as usize, piece.block.len() as u64);
        ...
        fn write_to_disk(
                ...
                .with_context(|| format!("error checking piece={index}"))?
            {
                true => {
                    ...
                    // bs004: log piece completion with summary stats
