...
impl PeerStates {
    ...
    /// Count how many connected peers have a given piece in their bitfield.
    pub fn count_peers_with_piece(&self, piece: usize) -> usize {
        let mut count = 0;
        for entry in self.states.iter() {
            let addr = *entry.key();
            if let Some(has_piece) = self.with_live(addr, |live| {
                live.bitfield.get(piece).map(|v| *v).unwrap_or(false)
            }) {
                if has_piece {
                    count += 1;
                }
